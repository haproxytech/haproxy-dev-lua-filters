...
/*************** Stack dumper for debugging purpose *****************/
static void hlua_print_item(lua_State *L, int i, int as_key);

static int hlua_is_identifier(const char *s) {
	while (*s) {
		if (!isalnum(*s) && *s != '_')
			return 0;
		++s;
...
static int hlua_is_seq(lua_State *L, int i) {
	int keynum = 1;
...
static void hlua_print_seq(lua_State *L, int i) {
	int k;
	...
	for (k = 1;; ++k) {
		...
		if (k > 1) printf(", ");
		hlua_print_item(L, -1, 0);  // 0 --> as_key
		lua_pop(L, 1);
		// stack = [..]
	}
        // stack = [.., nil]
...
static void hlua_print_table(lua_State *L, int i) {
	const char *prefix = "{";
	...
	if (hlua_is_seq(L, i))
		hlua_print_seq(L, i);  // This case includes all empty tables.
	else {
		// stack = [..]
		lua_pushnil(L);
		// stack = [.., nil]
		while (lua_next(L, i)) {
			...
			printf("%s", prefix);
			// stack = [.., key, value]
			hlua_print_item(L, -2, 1);  // 1 --> as_key
			printf(" = ");
...
static char *hlua_get_fn_string(lua_State *L, int i) {
...
static void hlua_print_item(lua_State *L, int i, int as_key) {
	...
	case LUA_TSTRING: {
		...
		if (hlua_is_identifier(s) && as_key)
			printf("%s", s);
		else
	...
	case LUA_TTABLE:
		printf("%s", first);
		hlua_print_table(L, i);
		printf("%s", last);
		return;

	case LUA_TFUNCTION:
		printf("%s%s%s", first, hlua_get_fn_string(L, i), last);
		return;
...
void hlua_print_stack(lua_State *L, int omit) {
	...
	for (i = 1; i <= n; ++i) {
		printf(" ");
		hlua_print_item(L, i, 0);  // 0 --> as_key
	}
	if (n == 0)
		printf(" <empty>");
	printf("\n");
 ...
 * copied (len != 0), it returns the length of the built string. If no data
 * are copied (len == 0), it returns -1 and push a nil value in the stack if the
 * channel's input is closed. Otherwise it returns 0.
 */
 ...
 * valid. If LF is found, the line is copied. If no LF is found and the
 * channel's input is closed, <len> bytes are copied. In both cases, the length
 * of the built string is returned. Otherwise nothing is copied, waiting for
 * more data and 0 is returned.
 */
static inline int _hlua_channel_dupline(struct channel *chn, lua_State *L, size_t offset, size_t len)
{
	...
	// No LF found, and the channel is not shutdown for reads, so wait
	if (!channel_input_closed(chn))
		return 0;
...
/* Appends as much data as possible from <str> to the channel's buffer at the
 * offset <offset>. This function returns -1 if nothing was copied while the
 * channel's input is closed. Otherwise, it returns the number of bytes
 * appended. */
static int _hlua_channel_append(struct channel *chn, lua_State *L, struct ist str, size_t offset)
{
	...
	/* Adjust the max we can copy, removing what was previously copied
	 * because of a yield */
	max = b_room(&chn->buf);
	if (max > str.len)
		max = str.len;

	ret = b_insert_blk(&chn->buf, offset, str.ptr, max);
	if (!ret) {
		/* Return an error if nothing was copied while the channel's
...
/* Helper function removing <len> bytes of data at the absolute position
 * <offset>.
 */
static void _hlua_channel_erase_data(struct channel *chn, size_t offset, size_t len)
{
...
__LJMP static int hlua_channel_send_yield(lua_State *L, int status, lua_KContext ctx)
{
	...
	ret = _hlua_channel_append(chn, L, ist2(str+l, sz - l), offset);
	if (ret == -1) {
		lua_pop(L, 1);
		lua_pushinteger(L, -1);
		return 1;
...
__LJMP static int hlua_http_msg_is_full(lua_State *L)
{
	...
	lua_pushboolean(L, (htx_free_data_space(htx) == 0));
	return 1;
}

/* Returns true if the HTTP message is empty. */
__LJMP static int hlua_http_msg_is_empty(lua_State *L)
{
	struct http_msg *msg;
	struct htx *htx;
	...
	lua_pushboolean(L, htx_is_empty(htx));
	return 1;
}

/* Returns the free space in the HTTP message. */
__LJMP static int hlua_http_msg_free_space(lua_State *L)
{
	struct http_msg *msg;
	struct htx *htx;

	MAY_LJMP(check_args(L, 1, "free_space"));
	msg = MAY_LJMP(hlua_checkhttpmsg(L, 1));
	htx = htxbuf(&msg->chn->buf);

	lua_pushinteger(L, htx_free_data_space(htx));
	return 1;
}
 ...
 * starting at the offset <offset> and put it in a string LUA variables. Returns
 * 0 if no data are available, otherwise it returns the length of the builded
 * string. It stops on the first non-DATA HTX block. This function is called
 * during the payload filtering, so the headers are already scheduled for output
 * (from the filter point of view). If nothing was copied, a nil value is pushed
 * on the stack. Otherwise a LUa string containing copied data is pushed on the
 * stack.
 */
...
/* Appends as much data as possible from <str> to the HTTP message <msg> at the
 * offset <offset>. It returns the amount of data written. This function is
 * responsibile to update the filter context.
 */
static int _hlua_http_msg_append(struct http_msg *msg, struct filter *filter, struct ist str, size_t offset)
{
	...
	/* Check if the buffer is available because HAProxy doesn't allocate
	 * the request buffer if its not required. */
	if (b_is_null(&msg->chn->buf)) {
		si_rx_buff_blk(chn_prod(msg->chn));
		goto end;
	}

	/* Adjust the max we can copy */
  ...
  fail:
	/* Return an error if nothing was copied while the channel's
	 * input is already closed */
	if (channel_input_closed(msg->chn))
		ret = -1;
	goto end;
}
 ...
 */
static void _hlua_http_msg_erase_data(struct http_msg *msg, struct filter *filter, size_t offset, size_t len)
{
	struct hlua_flt_ctx *flt_ctx = filter->ctx;
	struct htx *htx = htx_from_buf(&msg->chn->buf);
	struct htx_blk *blk;
...
/* "_hlua_http_msg_dup" wrapper. Unlike the channel function used to duplicate
 * raw data, this one can only be called inside a filter, from http_payload
 * callback. So it cannot yield. An exception is returned if it is called from
 * another callback. If nothing was copied, a nil value is pushed on the stack.
 */
__LJMP static int hlua_http_msg_dup(lua_State *L)
{
	struct http_msg *msg;
...
/* Does the same than hlua_http_msg_dup() except it removes data copied from the
 * HTTP message. Unlike the channel function used to get raw data, this one can
 * only be called inside a filter, from http_payload callback. So it cannot
 * yield. An exception is returned if it is called from another callback. If
 * nothing was copied, a nil value is pushed on the stack.
 */
__LJMP static int hlua_http_msg_get(lua_State *L)
{
	struct http_msg *msg;
	struct filter *filter;
	size_t offset, len;
	...
	ret = _hlua_http_msg_dup(msg, L, offset, len);
	if (ret)
		_hlua_http_msg_erase_data(msg, filter, offset, ret);
	return 1;
}

/* "_hlua_http_msg_append" wrapper. It returns the amount of data written or -1
 * if nothing was copied and the channel's input is closed. Unlike the channel
 * function used to duplicate raw data, this one can only be called inside a
...
__LJMP static int hlua_http_msg_append(lua_State *L)
{
	...
	ret = _hlua_http_msg_append(msg, filter, ist2(str, sz), offset+len);
	lua_pushinteger(L, ret);
	return 1;
}


/* Does the same than hlua_http_msg_append() except it first removes existing
 * data from the HTTP message (respecting the filter offset and length
 * values). The result is a replacement of the current data. It returns the
 * amount of data written or -1 if nothing was copied and the channel's input is
 * closed. Unlike the channel function used to duplicate raw data, this one can
 * only be called inside a filter, from http_payload callback. So it cannot
 * yield. An exception is returned if it is called from another callback.
 */
__LJMP static int hlua_http_msg_set(lua_State *L)
{
	struct http_msg *msg;
	struct filter *filter;
	const char *str;
	...
	if (len)
		_hlua_http_msg_erase_data(msg, filter, offset, len);
	ret = _hlua_http_msg_append(msg, filter, ist2(str, sz), offset);
	lua_pushinteger(L, ret);
	return 1;
}

/* Prepends as much data as possible from a string into an HTTP message at the
 * filter offset. It returns the amount of data written or -1 if nothing was
 * copied and the channel's input is closed. It also returns -1 if the channel's
 * output is closed. Unlike the channel function used to duplicate raw data,
 ...
 * channel function used to duplicate raw data, this one can only be called
 * inside a filter, from http_payload callback. So it cannot yield. An exception
 * is returned if it is called from another callback. All other functions deal
 * with DATA block, this one not.
*/
...
static int hlua_filter_new(struct stream *s, struct filter *filter)
{
	...
	if (!s->hlua) {
		s->hlua = pool_alloc(pool_head_hlua);
		if (!s->hlua || !hlua_ctx_init(s->hlua, s->task, 0)) {
	...
	flt_ctx = pool_alloc(pool_head_hlua_flt_ctx);
	if (!flt_ctx) {
	...
	memset(flt_ctx, 0, sizeof(*flt_ctx));
	flt_ctx->hlua[0] = pool_alloc(pool_head_hlua);
	flt_ctx->hlua[1] = pool_alloc(pool_head_hlua);
	if (!flt_ctx->hlua[0] || !hlua_ctx_init(flt_ctx->hlua[0], s->task, 0) ||
	    !flt_ctx->hlua[1] || !hlua_ctx_init(flt_ctx->hlua[1], s->task, 0)) {
		SEND_ERR(s->be, "Lua filter '%s': can't initialize filter Lua context.\n",
			 conf->name);
		ret = 0;
		goto end;
	...
	if (!HLUA_IS_RUNNING(s->hlua)) {
		...
		lua_rawgeti(s->hlua->T, LUA_REGISTRYINDEX, conf->ref);
	...
	case HLUA_E_OK:
		...
		break;
	case HLUA_E_ERRMSG:
		SEND_ERR(s->be, "Lua filter '%s' : %s.\n", conf->name, lua_tostring(s->hlua->T, -1));
		ret = -1;
		goto end;
	case HLUA_E_ETMOUT:
				...
				int dir, unsigned int flags)
{
	...
	case HLUA_E_AGAIN:
		...
		goto end;
	case HLUA_E_ERRMSG:
		SEND_ERR(s->be, "Lua filter '%s' : %s.\n", conf->name, lua_tostring(flt_hlua->T, -1));
		ret = -1;
		goto end;
	case HLUA_E_ETMOUT:
...
static int  hlua_filter_start_analyze(struct stream *s, struct filter *filter, struct channel *chn)
{
	...
	return hlua_filter_callback(s, filter, "start_analyze",
				    ...
				    (HLUA_FLT_CB_RETVAL | HLUA_FLT_CB_ARG_CHN));
}
...
static int  hlua_filter_http_headers(struct stream *s, struct filter *filter, struct http_msg *msg)
{
	...
	return hlua_filter_callback(s, filter, "http_headers",
				    ...
				    (HLUA_FLT_CB_RETVAL | HLUA_FLT_CB_ARG_HTTP_MSG));
}
				     ...
				     unsigned int offset, unsigned int len)
{
	...
	flt_ctx->flags |= HLUA_FLT_CTX_FL_PAYLOAD;
	if (flt_hlua && !HLUA_IS_RUNNING(flt_hlua)) {
		lua_pushinteger(flt_hlua->T, offset);
		lua_pushinteger(flt_hlua->T, len);
	}
				    ...
				    unsigned int offset, unsigned int len)
{
	...
	flt_ctx->cur_len[idx] = len;
	flt_ctx->flags |= HLUA_FLT_CTX_FL_PAYLOAD;
	if (s->hlua && !HLUA_IS_RUNNING(flt_hlua)) {
		lua_pushinteger(flt_hlua->T, offset);
		lua_pushinteger(flt_hlua->T, len);
	}
...
{
	struct hlua_flt_regconf *rconf = private;
	struct hlua_flt_config *conf = NULL;
	const char *flt_id = NULL;
	int error, pos, flt_ref, flt_flags = 0;
	...
	if (!hlua_flt_ops) {
		memprintf(err, "Lua filter '%s' : Lua out of memory error", rconf->name);
		return -1;
	}
	...
	lua_rawgeti(gL.T, LUA_REGISTRYINDEX, rconf->fun_ref);

	/* Push the filter class on the stack and resolve all callbacks */
	lua_rawgeti(gL.T, LUA_REGISTRYINDEX, rconf->flt_ref);
	if (lua_getfield(gL.T, -1, "start_analyze") == LUA_TFUNCTION)
		hlua_flt_ops->channel_start_analyze = hlua_filter_start_analyze;
	lua_pop(gL.T, 1);
	if (lua_getfield(gL.T, -1, "end_analyze") == LUA_TFUNCTION)
		hlua_flt_ops->channel_end_analyze = hlua_filter_end_analyze;
	lua_pop(gL.T, 1);
	if (lua_getfield(gL.T, -1, "http_headers") == LUA_TFUNCTION)
		hlua_flt_ops->http_headers = hlua_filter_http_headers;
	lua_pop(gL.T, 1);
	if (lua_getfield(gL.T, -1, "http_payload") == LUA_TFUNCTION)
		hlua_flt_ops->http_payload = hlua_filter_http_payload;
	lua_pop(gL.T, 1);
	if (lua_getfield(gL.T, -1, "http_end") == LUA_TFUNCTION)
		hlua_flt_ops->http_end = hlua_filter_http_end;
	lua_pop(gL.T, 1);
	if (lua_getfield(gL.T, -1, "tcp_payload") == LUA_TFUNCTION)
		hlua_flt_ops->tcp_payload = hlua_filter_tcp_payload;
	lua_pop(gL.T, 1);

	/* Get id and flags of the filter class */
	if (lua_getfield(gL.T, -1, "id") == LUA_TSTRING)
		flt_id  = lua_tostring(gL.T, -1);
	lua_pop(gL.T, 1);
	if (lua_getfield(gL.T, -1, "flags") == LUA_TNUMBER)
		flt_flags  = lua_tointeger(gL.T, -1);
	lua_pop(gL.T, 1);

	/* Duplicate the filter class so each filter will have its own copy */
	lua_newtable(gL.T);
        lua_pushnil(gL.T);
        while(lua_next(gL.T, 2)) {
		lua_pushvalue(gL.T, -2);
		lua_insert(gL.T, -2);
		lua_settable(gL.T, -4);
	}
	flt_ref = luaL_ref(gL.T, LUA_REGISTRYINDEX);

	/* Remove the original lua filter class from the stack */
	lua_pop(gL.T, 1);

	/* Push the copy on the stack */
	lua_rawgeti(gL.T, LUA_REGISTRYINDEX, flt_ref);

	/* extra args are pushed in a table */
	lua_newtable(gL.T);
	for (pos = *cur_arg + 1; *args[pos]; pos++) {
		/* Check stack available size. */
		if (!lua_checkstack(gL.T, 1)) {
			memprintf(err, "Lua filter '%s' : Lua error : full stack.", rconf->name);
			goto error;
		}
		lua_pushstring(gL.T, args[pos]);
		lua_rawseti(gL.T, -2, lua_rawlen(gL.T, -2) + 1);
	}
	*cur_arg = pos;

	error = lua_pcall(gL.T, 2, LUA_MULTRET, 0);
	switch (error) {
	case LUA_OK:
	...
	case LUA_ERRRUN:
		memprintf(err, "Lua filter '%s' : runtime error : %s", rconf->name, lua_tostring(gL.T, -1));
		goto error;
	case LUA_ERRMEM:
		memprintf(err, "Lua filter '%s' : out of memory error", rconf->name);
		goto error;
	case LUA_ERRERR:
		memprintf(err, "Lua filter '%s' : message handler error : %s", rconf->name, lua_tostring(gL.T, -1));
		goto error;
	case LUA_ERRGCMM:
		memprintf(err, "Lua filter '%s' : garbage collector error : %s", rconf->name, lua_tostring(gL.T, -1));
		goto error;
	default:
		memprintf(err, "Lua filter '%s' : unknonwn error : %s", rconf->name, lua_tostring(gL.T, -1));
		goto error;
	}
  ...
  error:
	luaL_unref(gL.T, LUA_REGISTRYINDEX, flt_ref);
	free(hlua_flt_ops);
	free(conf);
	lua_settop(gL.T, 0);
	return -1;
...
__LJMP static int hlua_register_filter(lua_State *L)
{
	...
	/* Use hlua_function to hide the ref on the filter class */
	rconf = calloc(1, sizeof(*rconf));
	if (!rconf) {
		free(fkl);
		WILL_LJMP(luaL_error(L, "Lua out of memory error."));
	}
	...
	/* Fill fcn. */
	rconf->name = strdup(name);
	if (!rconf->name) {
		free(fkl);
		free(rconf);
		WILL_LJMP(luaL_error(L, "Lua out of memory error."));
	}
	rconf->flt_ref = flt_ref;
	rconf->fun_ref = fun_ref;

	fkl->kw[0].parse = hlua_filter_parse_fct;
	fkl->kw[0].private = rconf;
...
#undef HLUA_PREPEND_PATH_TOSTRING1
	...
	hlua_class_function(gL.T, "register_filter", hlua_register_filter);
	...
	hlua_class_const_int(gL.T, "CONTINUE", 1);
	hlua_class_const_int(gL.T, "WAIT",     0);
	hlua_class_const_int(gL.T, "ERROR",    -1);

	hlua_class_const_int(gL.T, "FLT_CFG_FL_HTX", FLT_CFG_FL_HTX);

	hlua_class_function(gL.T, "wake_time", hlua_set_wake_time);
	hlua_class_function(gL.T, "register_data_filter", hlua_register_data_filter);
	hlua_class_function(gL.T, "unregister_data_filter", hlua_unregister_data_filter);

	lua_setglobal(gL.T, "filter");
	...
	/* Register Lua functions. */
	hlua_class_function(gL.T, "is_resp",     hlua_http_msg_is_resp);
	hlua_class_function(gL.T, "get_stline",  hlua_http_msg_get_stline);
	hlua_class_function(gL.T, "get_headers", hlua_http_msg_get_headers);
	hlua_class_function(gL.T, "del_header",  hlua_http_msg_del_hdr);
	hlua_class_function(gL.T, "rep_header",  hlua_http_msg_rep_hdr);
	hlua_class_function(gL.T, "rep_value",   hlua_http_msg_rep_val);
	hlua_class_function(gL.T, "add_header",  hlua_http_msg_add_hdr);
	hlua_class_function(gL.T, "set_header",  hlua_http_msg_set_hdr);
	hlua_class_function(gL.T, "set_method",  hlua_http_msg_set_meth);
	hlua_class_function(gL.T, "set_path",    hlua_http_msg_set_path);
	hlua_class_function(gL.T, "set_query",   hlua_http_msg_set_query);
	hlua_class_function(gL.T, "set_uri",     hlua_http_msg_set_uri);
	hlua_class_function(gL.T, "set_status",  hlua_http_msg_set_status);

	// TODO: find_header /  (full ou pas full)

	hlua_class_function(gL.T, "get",         hlua_http_msg_get);
	hlua_class_function(gL.T, "dup",         hlua_http_msg_dup);
	hlua_class_function(gL.T, "set",         hlua_http_msg_set);
	hlua_class_function(gL.T, "append",      hlua_http_msg_append);
	hlua_class_function(gL.T, "send",        hlua_http_msg_send);
	hlua_class_function(gL.T, "forward",     hlua_http_msg_forward);
	hlua_class_function(gL.T, "get_in_len",  hlua_http_msg_get_in_len);
	hlua_class_function(gL.T, "get_out_len", hlua_http_msg_get_out_len);
	hlua_class_function(gL.T, "is_full",     hlua_http_msg_is_full);
	hlua_class_function(gL.T, "is_emtpy",    hlua_http_msg_is_empty);
	hlua_class_function(gL.T, "free_space",  hlua_http_msg_free_space);

	hlua_class_function(gL.T, "blocks",      hlua_http_msg_blocks);

	hlua_class_function(gL.T, "first",       hlua_http_msg_first);
	hlua_class_function(gL.T, "next",        hlua_http_msg_next);

	lua_rawset(gL.T, -3);

	hlua_class_function(gL.T, "__len",       hlua_http_msg_nb_blocks);

	/* Register previous table in the registry with reference and named entry. */
	class_http_msg_ref = hlua_register_metatable(gL.T, CLASS_HTTP_MSG);
	...
	hlua_class_const_int(gL.T, "HTTP_BLOCK_REQ_SL", HTX_BLK_REQ_SL);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_RES_SL", HTX_BLK_RES_SL);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_HDR",    HTX_BLK_HDR);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_EOH",    HTX_BLK_EOH);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_DATA",   HTX_BLK_DATA);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_TLR",    HTX_BLK_TLR);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_EOT",    HTX_BLK_EOT);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_EOM",    HTX_BLK_EOM);
	hlua_class_const_int(gL.T, "HTTP_BLOCK_UNUSED", HTX_BLK_UNUSED);
	...
	hlua_class_function(gL.T, "remove",     hlua_http_blk_remove);
	hlua_class_function(gL.T, "set",        hlua_http_blk_set);
	hlua_class_function(gL.T, "replace",    hlua_http_blk_replace);
	hlua_class_function(gL.T, "insert",     hlua_http_blk_insert);
	hlua_class_function(gL.T, "append",     hlua_http_blk_append);
	hlua_class_function(gL.T, "prepend",    hlua_http_blk_prepend);

	// TODO: update de valeur de header (replace_header ou hdr_replace ou replace_hdr ...)
	// TODO: iter on  header_value  + remove ou replace de value

	hlua_class_function(gL.T, "data",       hlua_http_blk_data);
	...
	/* Register previous table in the registry with reference and named entry. */
	lua_pushvalue(gL.T, -1); /* Copy the -1 entry and push it on the stack. */
	class_http_blk_ref = hlua_register_metatable(gL.T, CLASS_HTTP_BLOCK);

	/* Assign the metatable to the HTTPBlock object. */
	lua_setmetatable(gL.T, -2);

	/* Set a name to the table. */
	lua_setglobal(gL.T, "HTTPBlock");
